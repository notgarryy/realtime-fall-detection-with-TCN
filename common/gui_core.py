...
class Window(QMainWindow):
    ...
    def __init__(self, parent=None, size=[], title="Applications Visualizer"):
        ...
        self.predLabel = QLabel("Prediction: -")
        self.core = Core()
        self.core.predictionReady.connect(self.predLabel.setText)
    ...
    def initConnectionPane(self):
        ...
        self.demoList.setCurrentIndex(1)
        self.demoList.setCurrentIndex(0)

        # --- Detect and set COM ports ---
        ...
        # ------------------------------
        # 📦 Add Prediction Display Box
        # ------------------------------
        self.predBox = QGroupBox("Live Activity Detection")
        self.predLayout = QGridLayout()

        self.predLabel = QLabel("No Data Yet")
        self.predLabel.setAlignment(Qt.AlignCenter)
        self.predLabel.setStyleSheet("""
            QLabel {
                font-size: 50px;
                font-weight: bold;
                color: #0078D7;
        ...
        """)
        self.predLayout.addWidget(self.predLabel)
        self.predBox.setLayout(self.predLayout)

        self.core.predictionReady.connect(lambda text: self.predLabel.setText(f"{text}"))

        # Tambahkan ke layout utama
        self.gridLayout.addWidget(self.predBox, 6, 0, 1, 1)
...
class Core(QObject):
    ...
    def handlePrediction(self, result_str):
    ...
    def connectCom(self, cliCom, dataCom, connectStatus):
        ...
        self.uart_thread.batchReady.connect(self.preprocess_thread.addBatch)
        self.preprocess_thread.preprocessedReady.connect(self.predict_thread.addPreprocessed)
        self.predict_thread.predictionReady.connect(self.handlePrediction)
    ...
    def parseData(self):
        # Start all processing threads when parsing begins
        self.uart_thread.start(priority=QThread.HighestPriority)
        self.preprocess_thread.start(priority=QThread.HighPriority)
        self.predict_thread.start(priority=QThread.NormalPriority)
    ...
    def gracefulReset(self):
        ...
        # Stop threads safely
        if hasattr(self, "uart_thread") and self.uart_thread.isRunning():
            self.uart_thread.stop()
        if hasattr(self, "preprocess_thread") and self.preprocess_thread.isRunning():
